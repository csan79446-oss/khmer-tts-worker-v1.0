...
| **`no kernel image available` / Worker unhealthy** | GPU ជំនាន់ Blackwell (B200, sm_100) — image ចាស់ (PyTorch 2.5.1+CUDA12.4) មិនគាំទ្រ | **ជម្រើស A (ងាយ):** Endpoint → Edit → GPU Types → deselect B200 → ប្រើ RTX 4090/A100/H100 ។ **ជម្រើស B:** Rebuild ជាមួយ image បច្ចុប្បន្ន (`pytorch/pytorch:2.7.1-cuda12.8-cudnn9-runtime` — CUDA 12.8 គាំទ្រ sm_100/Blackwell) |
...
| **Build បរាជ័យ `pip install ... exit code: 1`** | មូលហេតុអតីតកាលា: base image របស់ NGC ចាក់សោរ torch ជា pre-release (`2.7.0a0+<hash>`) និង export `PIP_CONSTRAINT` ដែលធ្វើឱ្យ package ថ្មីៗ resolve មិនបាន (`ResolutionImpossible`) | Dockerfile ឥឡូវប្រើ official `pytorch/pytorch:2.7.1-cuda12.8-cudnn9-runtime` (stable) — លែងមាន NGC constraint ឡើយ។ **មិនត្រូវបន្ថែម `pip install torchaudio`/`torch` ក្នុង Dockerfile ឡើយ** — rebuild ជាមួយ commit ថ្មី។ បើនៅតែបរាជ័យ ស្វែងរក `FATAL[stage=` ឬ traceback ក្នុង build log |
| **Build បរាជ័យ ឬ warning ពី `torchcodec`** | VoxCPM ប្រកាស `torchcodec` ក្នុង `pyproject.toml` ប៉ុន្តែ **គ្មាន .py ណាមួយ import វាទេ** — ហើយគ្មាន wheel សម្រាប់ torch 2.7.x ឡើយ | Dockerfile install `voxcpm` ដោយ `--no-deps` ដូច្នេះ `torchcodec` មិនត្រូវ install ទេ — មិនត្រូវបន្ថែមវាក្នុង `requirements.txt` ឡើយ |
...
| **Build ជោគជ័យ ប៉ុន្តែ log បង្ហាញ `WARNING: librosa and/or voxcpm did not import`** | dependency មួយមិនស៊ីជាមួយ torch ក្នុង image (traceback ពេញលេញបង្ហាញក្នុង build log) | ផ្ញើ traceback នោះមកដើម្បីកែ `requirements.txt` ។ បណ្តោះអាសន្ន worker ដំណើរការដោយ **Edge-TTS fallback** (មានស្លាក `[FALLBACK]` ក្នុង log និង field `engine` ក្នុង response) |
| **ចង់ប្តូរ version របស់ library** | `requirements.txt` ជា single source of truth — ភ្ជាប់តែ package ដែល import ជាក់លាក់តែប៉ុណ្ណោះ | កែ `requirements.txt` រួច rebuild — pins សំខាន់ៗ៖ `transformers<5`, `voxcpm @ ...@2.0.3`, `numpy<3`, `librosa<0.12` |
