...
| **Build បរាជ័យ `pip install ... exit code: 1`** | `pip install torchaudio` (ឬ `torch`) ក្នុង image នេះ៖ PyPI torchaudio កំណត់ `torch==2.7.0` ចំណែក torch ក្នុង NGC ជា pre-release `2.7.0a0+<hash>` ដែលចាក់សោរដោយ `PIP_CONSTRAINT` → pip ដោះស្រាយមិនបាន (`ResolutionImpossible`) ។ លើសពីនេះ `PIP_CONSTRAINT` របស់ NGC ចាក់សោរ package ទាំងអស់ (gradio 6 / datasets 3 / funasr / modelscope resolve មិនបាន) | Dockerfile ថ្មីសរសេរ constraint ឡើងវិញ ដោយចាក់សោរតែ `torch`/`torchaudio` ដែលមានក្នុង image ។ **មិនត្រូវបន្ថែម `pip install torchaudio`/`torch` ក្នុង Dockerfile ឡើយ** — rebuild ជាមួយ commit ថ្មី |
