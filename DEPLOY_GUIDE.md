...
| **Build ជោគជ័យ ប៉ុន្តែ log បង្ហាញ `WARNING: librosa and/or voxcpm did not import`** | dependency មួយមិនស៊ីជាមួយ torch ក្នុង image (traceback ពេញលេញបង្ហាញក្នុង build log) | ផ្ញើ traceback នោះមកដើម្បីកែ `constraints.txt` ។ បណ្តោះអាសន្ន worker ដំណើរការដោយ **Edge-TTS fallback** (មានស្លាក `[FALLBACK]` ក្នុង log និង field `engine` ក្នុង response) |
| **ចង់ប្តូរ version របស់ library** | `constraints.txt` ចាក់សោរ package ទាំងអស់តាម `uv.lock` ខាងក្នុង `voxcpm 2.0.3` | កែ `constraints.txt` (ឬ `requirements.txt`) រួច rebuild — មិនត្រូវលុប `constraints.txt` ចេញពី Dockerfile ឡើយ |
